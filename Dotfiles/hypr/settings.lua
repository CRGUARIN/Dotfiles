local col = require("colors")

-----------------------
---- LOOK AND FEEL ----
-----------------------

hl.config({
	general = {
		gaps_in = 5,
		gaps_out = 10,
		border_size = 2,
		col = {
			active_border = { colors = { col.primary, col.tertiary, col.primary_container }, angle = 45 },
			inactive_border = col.surface,
		},
		resize_on_border = false,
		allow_tearing = false,
		-- Options: dwindle, scrolling
		layout = "dwindle",
	},
	decoration = {
		rounding = 10,
		rounding_power = 2,
		dim_inactive = true,
		dim_strength = 0.3,
		active_opacity = 0.8,
		inactive_opacity = 0.65,
		shadow = {
			enabled = true,
			range = 20,
			render_power = 3,
			color = 0xee1a1a1a,
		},
		blur = {
			enabled = true,
			size = 6,
			passes = 2,
			vibrancy = 0.1696,
		},
	},
	animations = {
		enabled = true,
	},
	--scrolling = {
	-- Dirección en la que aparecen las ventanas ("right", "left", "up", "down")
	--	direction = "right",
	-- Ancho inicial por defecto de una nueva columna (0.5 es el 50% de la pantalla)
	--	column_width = 0.5,
	-- Si una columna está sola en el workspace, se expande a pantalla completa
	--	fullscreen_on_one_column = true,
	-- Mover la vista automáticamente a la ventana cuando gane el foco
	--	follow_focus = true,
	-- Lista de anchos preconfigurados para alternar rápidamente (ciclar)
	--	explicit_column_widths = "0.333, 0.5, 0.667, 1.0",
	-- Permitir que el foco vuelva al inicio al llegar al final de la cinta
	--	wrap_focus = true,
	--},
})

-----------------------------
---- CURVES & ANIMATIONS ----
-----------------------------

hl.curve("easeOutQuint", { type = "bezier", points = { { 0.23, 1 }, { 0.32, 1 } } })
hl.curve("easeInOutCubic", { type = "bezier", points = { { 0.65, 0.05 }, { 0.36, 1 } } })
hl.curve("linear", { type = "bezier", points = { { 0, 0 }, { 1, 1 } } })
hl.curve("almostLinear", { type = "bezier", points = { { 0.5, 0.5 }, { 0.75, 1 } } })
hl.curve("quick", { type = "bezier", points = { { 0.15, 0 }, { 0.1, 1 } } })
hl.curve("easy", { type = "spring", mass = 1, stiffness = 71.2633, dampening = 15.8273644 })

hl.animation({ leaf = "global", enabled = true, speed = 10, bezier = "default" })
hl.animation({ leaf = "border", enabled = true, speed = 10, bezier = "easeOutQuint" })
hl.animation({ leaf = "windows", enabled = true, speed = 4.79, spring = "easy" })
hl.animation({ leaf = "windowsIn", enabled = true, speed = 4.1, spring = "easy", style = "popin 87%" })
hl.animation({ leaf = "windowsOut", enabled = true, speed = 5, bezier = "linear", style = "popin 87%" })
hl.animation({ leaf = "fadeIn", enabled = true, speed = 1.73, bezier = "almostLinear" })
hl.animation({ leaf = "fadeOut", enabled = true, speed = 1.46, bezier = "almostLinear" })
hl.animation({ leaf = "fade", enabled = true, speed = 3.03, bezier = "quick" })
hl.animation({ leaf = "layers", enabled = true, speed = 3.81, bezier = "easeOutQuint" })
hl.animation({ leaf = "layersIn", enabled = true, speed = 4, bezier = "easeOutQuint", style = "fade" })
hl.animation({ leaf = "layersOut", enabled = true, speed = 1.5, bezier = "linear", style = "fade" })
hl.animation({ leaf = "fadeLayersIn", enabled = true, speed = 1.79, bezier = "almostLinear" })
hl.animation({ leaf = "fadeLayersOut", enabled = true, speed = 1.39, bezier = "almostLinear" })
hl.animation({ leaf = "workspaces", enabled = true, speed = 10, bezier = "default", style = "slide" })
hl.animation({ leaf = "zoomFactor", enabled = true, speed = 9, bezier = "quick" })
hl.animation({ leaf = "specialWorkspace", enabled = true, speed = 10, bezier = "default", style = "slidefadevert" })

------------------------
---- LAYOUTS & MISC ----
------------------------

hl.config({
	dwindle = { preserve_split = true },
	master = { new_status = "master" },
	scrolling = { fullscreen_on_one_column = true },
	misc = { force_default_wallpaper = -1, disable_hyprland_logo = false },
})

-------------------------
---- INPUT & DEVICES ----
-------------------------

hl.config({
	input = {
		kb_layout = "latam",
		kb_variant = "",
		kb_model = "",
		kb_options = "",
		kb_rules = "",
		follow_mouse = 1,
		sensitivity = 0,
		numlock_by_default = true,
		touchpad = { natural_scroll = true },
	},
})

hl.gesture({ fingers = 3, direction = "horizontal", action = "workspace" })
hl.device({ name = "epic-mouse-v1", sensitivity = -0.5 })
